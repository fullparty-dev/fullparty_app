...
export default function PartyPage() {
  ...
  const matchedParty = joinedParties.find(p => p.id === matchedPartyId);
  const isFullParty =
    matchedParty &&
    matchedParty.requireFull &&
    matchedParty.memberIds.length === matchedParty.maxMembers;
  const isPartialParty =
    matchedParty &&
    !matchedParty.requireFull &&
    matchedParty.memberIds.length >= 2;
  const isPartyEstablished = isFullParty || isPartialParty;
  ...
  return (
    <SmartphoneLayout>
      ...
      <div className="px-4 pb-32 pt-0 overflow-y-auto max-h-full">
        ...
        {isPartyEstablished && matchedParty && (
          <div className="mt-2"> {/* Added margin top */}
            <div className="bg-yellow-100 border border-yellow-300 text-yellow-800 p-4 rounded-xl shadow mb-4">
              ...
              <div className="border border-gray-200 rounded-xl p-3 mb-4 shadow bg-white text-black w-full max-w-xl text-sm">
                {(() => {
                  const emptySlots = matchedParty.maxMembers - matchedParty.memberIds.length;

                  const members = matchedParty.memberIds
                    .map(id => useUserStore.getState().getUserById?.(id))
                    .filter(Boolean)
                    .sort((a, b) => {
                      if (a.id === matchedParty.hostId) return -1;
                      if (b.id === matchedParty.hostId) return 1;
                      if (a.id === currentUserId) return 1;
                      if (b.id === currentUserId) return -1;
                      return 0;
                    });

                  const host = members.find(m => m.id === matchedParty.hostId);
                  const currentUser = members.find(m => m.id === currentUserId);
                  const others = members.filter(m => m.id !== matchedParty.hostId && m.id !== currentUserId);
